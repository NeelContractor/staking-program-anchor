...
const StakingCard: React.FC = () => {
  ...
  const handleCreatePda = async () => {
    ...
    if (!pdaExists) {
      // const success = await createPdaAccount();
          ...
          {(accounts.data ?? []).map((programAccount): React.ReactNode => {
            ...
            return (
              <div key={programAccount.publicKey.toString()}>
                <StakeAccountCard account={programAccount.publicKey} />
              </div>
            );
          })}
        </>
...
function StakeAccountCard({ account }: { account: PublicKey }) {
  ...
  const handleUnstake = async () => {
    ...
    //   showNotification('Insufficient staked amount', 'error');
    //   return;
    // }
  ...
  const handleClaimPoints = async () => {
    // if (stakedAmount <= 0) {
    //   showNotification('No staked tokens to claim points from', 'error');
    //   return;
    // }
